...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  let lastDark=isDark();
  renderer.setPixelRatio(Math.min(devicePixelRatio,innerWidth<768?1.5:2));renderer.setClearColor(lastDark?'#0f1626':'#f2f3f3');renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;el.appendChild(renderer.domElement);
  ...
  const ground=new T.Mesh(new T.CircleGeometry(30,96),new T.MeshStandardMaterial({color:lastDark?0x0d1320:0xd5d9dc,roughness:1}));ground.rotation.x=-Math.PI/2;ground.position.y=-.019;scene.add(ground);
  const platform=new T.Mesh(new T.CylinderGeometry(.68,.7,.028,100),new T.MeshStandardMaterial({color:lastDark?0x1a2540:0xeeeeec,metalness:.12,roughness:.67}));platform.position.y=-.016;scene.add(platform);
  const applyTheme=(d:boolean)=>{renderer.setClearColor(d?'#0f1626':'#f2f3f3');(ground.material as T.MeshStandardMaterial).color.set(d?0x0d1320:0xd5d9dc);(platform.material as T.MeshStandardMaterial).color.set(d?0x1a2540:0xeeeeec);dirty=true;};
  ...
  const animate=()=>{
   ...
   {const d=isDark();if(d!==lastDark){lastDark=d;applyTheme(d);}}
